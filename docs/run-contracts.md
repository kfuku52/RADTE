...
Retries may change optimizer controls, starting seeds, input edge scaling, model, or lambda. They never widen exact ages, lower bounds, or turn hard constraints into soft bounds. `--allow_constraint_drop=true` permits RS → S → R selection within each control profile. The fast profile runs first; a fast S or R success ends the search before high-cost RS is attempted. Only after every fast stage fails does the high-cost profile repeat the sequence, if enabled and budget remains. With `false`, only RS is tried in each profile; a deterministic feasible tree may be returned when optimization fails. Its manifest has `chronos_model_used=deterministic-fallback` and no effective random seed. That tree is a constraint-respecting construction, not a maximum-likelihood or posterior estimate.

Chronos results must have finite positive edges, the same topology and tips as the gene input, ultrametric tips, and ages inside retained bounds. Age/ultrametric comparison uses `1e-7 * max(1, tree_height)` for floating-point tolerance. Feasibility requires positive edges; the default numerical margin is `16 * .Machine$double.eps * max(1, maximum_calibration_age)`. For either backend, `--pad_short_edge` first floors input gene branches in substitution-per-site units. Chronos additionally uses the same numeric threshold to project dated edges in time units within the same hard bounds, failing if that minimum is infeasible. It preserves existing inferred ages where feasible and records `branch_padding_applied`. MCMCTree output is not padded. Species-tree calibration ages are never changed by padding.
...
Finite chronos attempts run in a child process on Unix, allowing termination on timeout. Windows uses R's `setTimeLimit()`, so long compiled computations can overrun the budget until they reach an interrupt check. The total budget covers chronos retries, not preprocessing, deterministic construction, or publication; it is not a whole-run deadline. See the [timeout options](usage.md#--chronos_attempt_timeout_sec).
...
Executable paths are made absolute while preserving the final symlink name. The [Debian PAML wrapper](https://sources.debian.org/src/paml/4.9j%2Bdfsg-4/debian/bin/baseml/) chooses its program from that name, so invoking the fully resolved target would select a different tool. Collision checks and file hashes still resolve symlinks; `mcmctree_executable` records the actual invocation path.
